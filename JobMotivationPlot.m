function [] = JobMotivationPlot(file_name, headings)
    ...
    for i = 1 : num_students
        ...
        %if no match was found against any of the motivations, increment the 'Other' count
        if ~found
    ...
    for i = 1 : length(motivation_counters)
        if round((motivation_counters(i) / sum(motivation_counters)) * 100, 2) ~= 0
            options_index = options_index + 1;
            final_motivations{options_index} = motivations{i};
            motivation_proportions(options_index) = round((motivation_counters(i) / sum(motivation_counters)) * 100, 2);
        end
    end
